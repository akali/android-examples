...
public class MainActivity extends AppCompatActivity {
	private static final int LIST_ACTIVITY_REQUEST = 1;
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == LIST_ACTIVITY_REQUEST) {
			onListActivityResult(resultCode, data);
		}
	}

	private void onListActivityResult(int resultCode, Intent data) {
		if (resultCode == RESULT_OK) {
			int number = data.getIntExtra("data", 0);
			Toast.makeText(this, number + " have been touched", Toast.LENGTH_SHORT).show();
		}
	}
}
